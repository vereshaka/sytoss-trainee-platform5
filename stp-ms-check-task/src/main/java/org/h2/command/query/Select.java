...
public class Select extends Query {
    ...
    void updateAgg(int columnCount, int stage) {
        Map<Column, List<Integer>> columns = new HashMap<>();
        for (int i = 0; i < columnCount; i++) {
            Expression expr = expressions.get(i);
            if (expr instanceof ExpressionColumn) {
                Column column = ((ExpressionColumn) expressions.get(i)).getColumn();
        ...
        for (int i = 0; i < columnCount; i++) {
            if ((groupByExpression == null || !groupByExpression[i])
                ...
                if (expr instanceof ExpressionColumn) {
                    Column column = ((ExpressionColumn) expressions.get(i)).getColumn();
                    List<Integer> groups = columns.get(column);
                    boolean result = false;
                    for (int group : groups) {
                        result = result || groupByExpression[group];
                    }
                    if (!result) {
                        throw DbException.get(ErrorCode.MUST_GROUP_BY_COLUMN_1, expr.getTraceSQL());
                    }
                }
