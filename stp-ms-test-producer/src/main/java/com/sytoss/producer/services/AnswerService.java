...
public class AnswerService {
    ...
    void checkAnswer(Answer answer, PersonalExam personalExam) {
        ...
        TaskDomain taskDomain = metadataConnector.getTaskDomain(findTaskDomainByTask(answer.getTask().getId()));
    ...
    public Long findTaskDomainByTask(Long answerTaskId) {
        Long TaskDomainId = null;
        ...
        for (TaskDomainDTO taskDomainDTO:taskDomainList) {
            for (TaskDTO taskDTO:taskDomainDTO.getTasks()) {
                if (taskDTO.getId().equals(answerTaskId)) {
                    TaskDomainId=taskDomainDTO.getId();
                    break;
                }
            }
        }
