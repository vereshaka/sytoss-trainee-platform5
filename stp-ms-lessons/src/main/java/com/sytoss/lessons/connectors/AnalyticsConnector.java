...
public interface AnalyticsConnector extends CrudRepository<AnalyticsDTO, Long> {
    ...
    @Query("SELECT new com.sytoss.lessons.dto.AnalyticsAverageDTO(a.studentId, AVG(a.grade), AVG(a.timeSpent), row_number() over (order by AVG(a.grade) desc, AVG(a.timeSpent))) from ANALYTICS a where a.disciplineId = :disciplineId group by a.studentId order by 2 desc, 3 asc")
    List<AnalyticsAverageDTO> getStudentRatingsByDiscipline(@Param("disciplineId") Long disciplineId);

    @Query("SELECT new com.sytoss.lessons.dto.AnalyticsAverageDTO(a.studentId, AVG(a.grade), AVG(a.timeSpent),  row_number() over (order by AVG(a.grade) desc, AVG(a.timeSpent))) from ANALYTICS a where a.disciplineId = :disciplineId and a.studentId in :studentIds group by a.studentId order by 2 desc, 3 asc")
    ...
    @Query("SELECT new com.sytoss.lessons.dto.AnalyticsAverageDTO(a.studentId, AVG(a.grade), AVG(a.timeSpent),  row_number() over (order by AVG(a.grade) desc , AVG(a.timeSpent))) from ANALYTICS a where a.disciplineId = :disciplineId and a.examId = :examId group by a.studentId order by 2 desc, 3 asc")
